...
def surge_extraction(dataset, log_templates, length, duration):
    """
    计算突变特征
    """
    features = np.array(pd.DataFrame(columns=range(log_templates)))
    for _, row in tqdm(dataset.iterrows(), desc="surge extraction:"):
        time_sequences = generate_time_sequence(row, log_templates, duration)
        surge_feature = np.array(pd.DataFrame(columns=range(length/duration)))
